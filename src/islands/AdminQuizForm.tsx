...
export default function AdminQuizForm({ moduleNumber, question, mode }: AdminQuizFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <label htmlFor="quiz-question" style={labelStyle}>
          Pergunta *
        </label>
        <textarea
          id="quiz-question"
      ...
      <fieldset className="space-y-2" style={{ border: "none", padding: 0, margin: 0 }}>
        ...
        {options.map((opt, i) => (
          <div key={i} className="flex items-center gap-3">
            ...
            />
            <span className="font-mono text-xs" style={{ color: "var(--fade)", minWidth: "1rem" }}>
              {LABELS[i]}
            </span>
            <input
              type="text"
      ...
      <div>
        <label htmlFor="quiz-order" style={labelStyle}>
          Ordem
        </label>
        <input
          id="quiz-order"
      ...
      <div className="flex gap-3 pt-2">
        ...
        <a href={`/admin/quiz/${moduleNumber}`} className="btn-ghost">
          Cancelar
        </a>
        {mode === "edit" && (
          <button
