...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  if (!params.module || Number.isNaN(moduleNumber) || moduleNumber < 1)
  ...
  if (!Array.isArray(body.answers))
    return new Response(JSON.stringify({ error: "answers deve ser um array" }), {
      status: 400,
      headers: { "Content-Type": "application/json" },
    });
  ...
  const validAnswers = answers.every(
    (a) => Number.isInteger(a) && (a as number) >= 0 && (a as number) <= 3,
  );
  if (!validAnswers)
    return new Response(
      JSON.stringify({ error: "Cada resposta deve ser um inteiro entre 0 e 3" }),
  ...
  try {
    ...
    if (attemptCount >= MAX_QUIZ_ATTEMPTS)
      return new Response(JSON.stringify({ error: "Limite de tentativas atingido" }), {
        status: 403,
        headers: { "Content-Type": "application/json" },
      });
