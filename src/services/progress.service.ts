...
export class ProgressService {
  ...
  static async getLessonProgress(userId: string, lessonId: string): Promise<UserProgress | null> {
    const { data, error } = await supabaseAdmin
      .from("user_progress")
      .select("*")
      .eq("user_id", userId)
  ...
  static async updateWatchTime(userId: string, lessonId: string, watchTime: number): Promise<void> {
    const { error } = await supabaseAdmin.from("user_progress").upsert(
      {
        user_id: userId,
        lesson_id: lessonId,
