...
export class OrdersService {
  ...
  static async hasActiveAccess(userId: string): Promise<boolean> {
    try {
      ...
      if (error) {
        logger.error("orders.hasActiveAccess failed", {
          userId,
          code: error.code,
      ...
      logger.error("orders.hasActiveAccess threw", {
        userId,
  ...
  static async getLatestByUserId(userId: string): Promise<Order | null> {
    ...
    try {
      ...
      if (error) {
        logger.error("orders.getLatestByUserId failed", {
          userId,
          code: error.code,
          message: error.message,
        });
      ...
      logger.error("orders.getLatestByUserId threw", {
        userId,
        err: err instanceof Error ? err.message : String(err),
      });
      return null;
  ...
  static async create(input: Omit<Order, "id" | "created_at">): Promise<Order> {
    const { data, error } = await supabaseAdmin.from("orders").insert(input).select().single();
