...
## Bundle Size Baseline (2026-05-05)

Adapter Vercel SSR — output em `dist/client/_astro/`. Limite no CI: **500KB**.

| Arquivo | Tamanho |
|---|---|
| `client.js` (React + Supabase shared) | 182.2KB |
| `ResetPasswordForm.js` (Supabase SSR) | 179.8KB |
| `VideoPlayer.js` | 27.1KB |
| `index.js` | 7.7KB |
| demais islands | < 5KB cada |
| **TOTAL** | **428.5KB** |

`ResetPasswordForm` é grande por importar `@supabase/ssr` (`createBrowserClient`) — esperado para fluxo de auth.
...
- Implementação no projeto: `scripts/quality-gate.js` + `.github/workflows/ci.yml`
